        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                for(i=0;i<x.length;i++)
                {
                    ...
                    output+='<img class="card-img-top mt-5 p-4" src="../Admin/uploads/'+x[i].stud_image+'" alt="Card image cap">';
                    output+='</div>';
                    output+=' <div class="col-sm-9">';
                    output+='<div class="card-body">';
                    output+='<h5 class="card-title">'+x[i].stud_name+'</h5>';
                    output+='<p class="card-text">';
                    output+=''+x[i].stud_name+'';
                    output+='</h5>';
                    output+='<h6 class="card-subtitle mb-2 text-muted">';
                    output+=''+x[i].regno+'';
                    output+='</h6>';
                    output+='<p class="card-text">';
                    output+='Email:'+x[i].stud_email+'';
                    output+='<br>';
                    output+='Gender:'+x[i].stud_gender+'';
                    output+='<br>';
                    output+='Address:';
                    output+=''+x[i].stud_address+'';
                    output+='</p>';
                    output+='</div>';
                    output+='</div>';
                    output+='</div>';
